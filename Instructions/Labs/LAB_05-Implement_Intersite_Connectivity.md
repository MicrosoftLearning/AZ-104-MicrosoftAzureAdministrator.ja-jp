...
lab:
  ...
  description: 仮想ネットワーク、ルート、ピアリングを作成して構成します。
  duration: 50 minutes
  level: 400
  islab: true
  primarytopics:
...
1. [Virtual Machines] ページで、**[作成]** を選択し、**[仮想マシン]** を選択します。
...
1. **[ディスク]** タブ上で既定値のまま、**[次へ: ネットワーク >]** を選択します。
...
1. **[診断テストを実行する]** を選択してください。
    ...
    >**注**:結果が返されるまでに数分かかる場合があります。 結果が収集されている間、画面の選択項目は灰色表示されます。 **[接続テスト]** に **[到達不可能]** と表示されることに注意してください。 この仮想マシンは別の仮想ネットワーク内にあるため、これは当然です。 
...
1. `CoreServicesVM` 仮想マシンに切り替えます。 **[演算]** ブレードで **[コマンドの実行]** を選択してから、**[RunPowerShellScript]** を選択します。 受信 RDP トラフィックを許可する Windows ファイアウォール規則を有効にするには、次のコマンドを実行します。

    ```Powershell
    Enable-NetFirewallRule -DisplayGroup "Remote Desktop"
    ```

1. スクリプトが完了するのを待ってから、`ManufacturingVM` 仮想マシンに切り替えます。
...
1. **[RunPowerShellScript]** を選択し、「**Test-NetConnection**」コマンドを実行します。 必ず **CoreServicesVM** のプライベート IP アドレスを使用してください。
    ...
    ```
1. スクリプトがタイムアウトするまでに数分かかる場合があります。ページの上部に "スクリプトを実行しています..." という情報メッセージが表示されます。**
...
このタスクでは、境界サブネットと内部の主要サービスのサブネット間のネットワーク トラフィックを制御します。 仮想ネットワーク アプライアンスは境界サブネットにインストールされます。また、すべてのトラフィックがそこにルーティングされるようにする必要があります。 

1. `CoreServicesVnet` を検索して選択します。

1. **[サブネット]** を選択してから、**[+ サブネット]** をクリックします。 必ず **[追加]** を選択して変更を保存してください。 
...
1. 次の詳細を入力し、**[確認および作成]** を選択して、**[作成]** を選択します。 
...
1. ルート テーブルがデプロイされたら、**[ルート テーブル]** を検索して選択します。
   
1. **[rt-CoreServices]** のリソース (チェックボックスではなく) を選択します。

1. **[設定]** を展開して **[ルート]** を選択し、**[+ 追加]** を選択します。 将来のネットワーク仮想アプライアンス (NVA) から CoreServices 仮想ネットワークへのルートを作成します。 
...
1. **[追加]** を選択します。 最後に、そのルートをサブネットに関連付けます。

1. **[サブネット]**、**[+ 関連付け]** の順に選択します。 構成を完了します。

    | 設定 | 値 | 
    | --- | --- |
    | 仮想ネットワーク | **CoreServicesVnet (az104-rg5)** |
    | Subnet | **境界** |    
...
+ Azure portal で、リソース グループを選び、**[リソース グループの削除]**、**[リソース グループ名を入力してください]** の順に選び、**[削除]** をクリックします。 リソース グループの削除は永続的であり、元に戻すことができないことを示す **[削除の確認]** ダイアログボックスが表示されたら、もう一度 **[削除]** をクリックして削除を完了します。
...
+ [仮想ネットワークの概要](https://learn.microsoft.com/training/modules/introduction-to-azure-virtual-networks/)。 このモジュールでは、Azure ネットワーク サービスを設計および実装する方法について説明します。 仮想ネットワーク、パブリック IP とプライベート IP、DNS、仮想ネットワーク ピアリング、ルーティング、Azure Virtual Network NAT などを取り上げます。
