...
lab:
  ...
  description: テンプレートを使用して Azure リソースをデプロイします。
  duration: 50 minutes
  level: 400
  islab: true
  primarytopics:
...
1. **[マネージド ディスクの作成]** ページで、ディスクを構成し、**[OK]** を選択します。 
    ...
    >**注:** 容量やクォータの制限でデプロイが失敗した場合は、構成を調整するか別のリージョンを選択してください。
...
1. **[テンプレート]** セクションで、**[ダウンロード]** をクリックし、テンプレートをローカル ドライブに保存します。 次に、**[パラメーター]** セクションに切り替えて、同じ操作を行います。 

1. エクスプローラーを使用して、コンピューター上の **[ダウンロード]** フォルダーを開きます。 JSON ファイルが 2 つ (テンプレートとパラメータ) あることを確認してください。 
...
1. Cloud Shell は一時的な PowerShell セッションに直接起動することもあります。 ストレージ アカウントの構成は不要です。

    >**注:** PowerShell プロンプトが表示されるまで待ってから進めてください。
...
1. 変更を加えます。 たとえば、ディスク名を **[az104-disk3]** に変更します。 **Ctrl + S** キーを押して変更を保存してから、**Ctrl + Q** キーを押してエディターを閉じます。 

    >**注**:リソース グループ、サブスクリプション、管理グループ、またはテナントをテンプレートのデプロイのターゲットにすることができます。 使用するコマンドは、デプロイのスコープに応じて異なります。

1. 複数のサブスクリプションがある場合は、最初に PowerShell が正しいサブスクリプション コンテキストを使用していることを確認してください。 次のコマンドを実行し、必要に応じて `<your-subscription-id>` を **az104-rg3** を含むサブスクリプションに置き換えます。
    ...
    >**注:** Cloud Shell がエフェメラル モードで動作している場合、アクティブなサブスクリプションが Azure portal のコンテキストと異なる可能性があるため、これは特に重要です。
...
1. 複数のサブスクリプションがある場合は、まず `az account show` を実行してアクティブなサブスクリプションを確認し、正しいサブスクリプション コンテキストが設定されていることを確認してください。 **az104-rg3** を含むサブスクリプションと一致しない場合は、先に進む前に `az account set --subscription <your-subscription-id>` を実行してください。 Cloud Shell がエフェメラル モードで動作している場合、PowerShell から Bash への切り替えによってサブスクリプション コンテキストがリセットされる可能性があるため、これは特に重要です。
...
1. 変更を加えます。 たとえば、ディスク名を **[az104-disk4]** に変更します。 **Ctrl + S** キーを押して変更を保存してから、**Ctrl + Q** キーを押してエディターを閉じます。 
...
1. **[\Allfiles\Lab03\azuredeploydisk.bicep]** ファイルを見つけます。

1. **[Bash]** セッションの **[Cloud Shell]** で作業を続けます。

1. **[ファイルのアップロード/ダウンロード]** を選択してから、Cloud Shell に Bicep ファイルを**アップロード**します。 

1. **[エディターを開く]** を選択します。

1. **azuredeploydisk.bicep** ファイルを選択します 
...
1. **Ctrl + S** キーを押して変更を保存してから、**Ctrl + Q** キーを押してエディターを閉じます。
...
+ Azure portal で、リソース グループを選び、**[リソース グループの削除]**、**[リソース グループ名を入力してください]** の順に選び、**[削除]** をクリックします。 リソース グループの削除は永続的であり、元に戻すことができないことを示す確認ダイアログが表示されたら、もう一度 **[削除]** をクリックして削除を完了します。
...
+ [Azure CLI を使用して Azure リソースを作成します](https://learn.microsoft.com/training/modules/create-azure-resources-by-using-azure-cli/)。 Windows、Linux、macOS 全体に Azure CLI をインストールする方法、コマンドを対話形式で実行する方法、Bash オートメーション スクリプトを作成する方法、一般的な問題をトラブルシューティングする方法について学習します。 
