...
lab:
  ...
  description: Azure の Web アプリを作成してスケーリングします。
  duration: 20 minutes
  level: 300
  islab: true
  primarytopics:
...
このタスクでは、Azure Web アプリを作成します。 Azure アプリ Service は、Web、モバイル、その他の Web ベースのアプリケーション用のサービスとしてのプラットフォーム (PAAS) ソリューションです。 Azure Web アプリは、PHP、Java、.NET などのほとんどのランタイム環境をホストする Azure アプリ Service の一部です。 選択した App Service プランによって、Web アプリのコンピューティング、ストレージ、機能が決まります。 
...
1. **[追加]** を選択して、新しいスロットを作成します。

1. ページを更新して、運用スロットとステージング スロットを表示します。 

1. 新しく作成されたステージング スロットを表すエントリを選択します。
...
1. **ステージング** スロットで、**[構成]**、**[全般設定]** の順に選択します。

    >**注:**  運用スロットではなく、ステージング スロットのブレードであることを確認します。

1. **[SCM 基本認証発行資格情報]** で、チェックボックスをオンにして **[適用]** を選択します。 次に、**[デプロイ センター]**、**[設定]** の順に選択します。

1. "お使いのアプリでは、SCM 基本認証が無効になっています" という警告バナーが表示された場合は、**[ここで有効にする]** を選択し、ステップを完了して有効にします。 
...
1. 既定の設定を確認し、**[スワップの開始]** をクリックします。 スワップが完了したことを示す通知を待ちます。

1. ポータルのホーム ページに戻ります。 運用 Web アプリとステージング スロットの両方が必要です。

1. `App Services` を検索し、お使いの App Service Web アプリを選択します。 これにより、実稼働デプロイ スロットに戻ります。

1. App Service の Web アプリを選択し、Web アプリの **[概要]** ブレードで、**[既定のドメイン]** リンクを選択して、Web サイトのホーム ページを表示します。

1. 運用 Web ページに **Hello World!** が表示されていることを確認します。 のページを参照してください。
...
1. 左側のペインの **[App Service プラン]** セクションで、**[スケールアウト]** を選びます。
...
1. **[問題の診断と解決]** (Web アプリのメイン ページの左側のペイン) を選びます。
...
1. **[概要]** | **[HTTP 要求を追加して作成]** から、**[作成]** を選択します。

1. **[テスト計画]** タブで、**[要求の追加]** を選択します。 **[URL フィールド]** に、お使いの**既定のドメイン** URL を貼り付けます。 正しく書式設定され、**https://** で始まっていることを確認します。 **[追加]** を選択して変更を保存します。 

1. **[確認と作成]**、**[作成]** の順に選択します。

    >**注:**  テストの作成には数分かかる場合があります。 通知を確認します。

1. テストに移動します (ホーム ページに表示されます)。 

1. **仮想ユーザー**、**応答時間**、**要求/秒**などのライブ メトリックを更新して確認します。
...
+ Azure portal で、リソース グループを選び、**[リソース グループの削除]**、**[リソース グループ名を入力してください]** の順に選び、**[削除]** をクリックします。 2 回目の [削除の確認] ダイアログが表示されたら、もう一度 **[削除]** をクリックして削除を完了します。
...
+ [Azure App Service で Web アプリケーションをホストする](https://learn.microsoft.com/en-us/training/modules/host-a-web-app-with-azure-app-service/)。 Azure App Service でホストされる Web アプリのプラットフォームを使って Web サイトを作成する方法について説明します。
+ [Web アプリの設定を構成します](https://learn.microsoft.com/en-us/training/modules/configure-web-app-settings/)。 アプリケーション設定の作成と管理、Web トラフィックをセキュリティで保護するための SSL/TLS 証明書のインストール、診断ログの有効化、ディレクトリへの仮想アプリのマッピングの作成、アプリ機能の管理を行う方法について学習します。
  
## 要点

以上でラボは完了です。 このラボの要点は次のとおりです。 

+ Azure アプリ Service を使用すると、Web アプリの構築、デプロイ、スケーリングをすばやく行える。
