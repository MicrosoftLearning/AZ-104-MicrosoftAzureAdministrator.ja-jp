...
1. **「仮想マシンの作成」** ブレードの **「基本」** タブで、次の設定を指定します (他の設定は既定値のままにします)。
    ...
    >**注**: リージョンやタイミングによっては**Standard D2s v3**が使用できない場合があります。その場合は使用できる他のサイズを選択してください。（クレジットを大量に消費してしまうので、あまり大きなサイズは選択しないでください）
