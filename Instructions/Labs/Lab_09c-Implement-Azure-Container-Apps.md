...
lab:
  ...
  description: Azure Container Apps を実装してデプロイします。
  duration: 15 minutes
  level: 300
  islab: true
  primarytopics:
...
1. 以下の情報を使用して **[基本]** タブの詳細を設定します。
    ...
    | コンテナー アプリ名 |  `my-app` |
    | リージョン    | **米国東部** |
    | Container Apps 環境 | **[新しい環境の作成]** を選択し、環境名を `my-environment` に設定して  > **[作成]** します。 |

1. **[次へ: コンテナー]** タブをクリックし、**[クイックスタート イメージの使用]** がオンになっていることを確認します。 この設定を表示するには、上にスクロールすることが必要な場合があります。 

1. **[クイックスタート イメージ]** を **[単純な Hello World コンテナー]** に設定したままにします。

1. **[アプリケーションのイングレス設定]** で、必要に応じてイングレス オプションを構成し、**[次へ: タグ]** をクリックします。
...
1. **[Your container app is running with a Hello World image]\(お使いのコンテナー アプリは Hello World イメージで実行されています\)** というメッセージを受信したことを確認します。
...
+ [Azure Container Apps を実装する](https://learn.microsoft.com/training/modules/implement-azure-container-apps/)。 サーバーレス プラットフォーム上のマイクロサービスとコンテナー化されたアプリのデプロイと管理に、Azure Container Apps がどのように役立つかを学習します。
