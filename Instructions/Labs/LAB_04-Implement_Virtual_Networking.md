...
lab:
  ...
  description: 仮想ネットワーク、ネットワーク セキュリティ グループ、DNS ゾーンを構成します。
  duration: 50 minutes
  level: 400
  islab: true
  primarytopics:
...
1. **+ サブネットの追加** を選択します。 各サブネットの名前とアドレスの情報を入力します。 必ず新しいサブネットごとに **[追加]** を選択します。 
    ...
    >**注:**  他のサブネットを作成する前または作成後に、既定のサブネットを削除することを忘れないでください。
...
1. **[テンプレート]** タブを選択し、テンプレートを**ダウンロード**します。 次に、**[パラメーター]** タブに切り替え、**ダウンロード**操作を繰り返します。

1. ローカル コンピューター上の **[ダウンロード]** フォルダーに移動します。 
...
1. **AllowInternetOutBound** 規則に注目してください。 また、この規則は削除できず、優先度が 65001 であることにも注意してください。
...
1. **[基本]** タブを構成します。
    ...
    | リソース グループ | **az104-rg4** |
    | Name | `contoso.com` (この名前は一意である必要があります。contoso.com は予約されているため、他の名前に変更してください。) |
...
1. **[概要]** ブレードで、ゾーンに割り当てられている 4 つの Azure DNS ネーム サーバーの名前を確認します。 ネーム サーバーのアドレスのいずれかを**コピー**します。 これは、以下の nslookup コマンドの今後の手順で必要になります。 
...
1. コマンド プロンプトを開いて、次のコマンドを実行します。 ドメイン名を変更した場合は、調整を行います。 
   ...
   ```
1. ホスト名 www.contosoxyz104.com が指定した IP アドレスに解決されることを確認します。 これにより、名前解決が正常に機能していることを確認します。
...
1. [プライベート DNS ゾーンの作成] の **[基本]** タブで、次の表に示すように情報を入力します。
    ...
    | サブスクリプション | **サブスクリプションを選択します** |
    | リソース グループ | **az104-rg4** |
    | Name | `private.contoso.com` (名前を変更する必要がある場合は調整します) |
    | リージョン |**米国東部** |
