...
1. **[Next : IP Addresses](次へ: IP アドレス)** をクリックします。 **開始アドレス**は **10.40.0.0 です**。 **アドレス空間のサイズ**は、 **/20** です。 

1. **[+ サブネットの追加]** をクリックします。 既存の **default** サブネットを削除します。 次の値を入力し、**[追加]** をクリックします。 
...
1. **[パブリック IP アドレスの関連付け]** を選択し、 **[パブリック IP アドレス]** ドロップダウンで **az104-04-pip0** を選択します。
    ...
    >**注:**  エラー "ドメイン名が既に使用されています" が発生した場合、これは既知の問題です。** パブリック IP アドレスを見つけて、それを NIC に個別に関連付ける必要があります。
    >
    > + **[パブリック IP アドレス]** に移動します
    > + **[az104-04-pip0]** をクリックします
    > + **[概要]** ペインで、**[IP の関連付け]** をクリックします
    > + **[リソースの種類]** を **[ネットワーク インターフェイス]** に設定します
    > + **[ネットワーク インターフェイス]** を **[az104-04-nic0]** に設定します
    > + **[az104-04-pip1]** と **[az104-04-nic1]** についても繰り返します
