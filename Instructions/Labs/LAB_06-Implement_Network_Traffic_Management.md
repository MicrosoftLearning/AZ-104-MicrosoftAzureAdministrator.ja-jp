---
lab:
  title: 'ラボ 06: ネットワーク トラフィック管理を実装する'
  module: Administer Network Traffic Management
  description: Azure Load Balancer と Application Gateway を作成して構成します。
  duration: 50 minutes
  level: 400
  islab: true
  primarytopics:
...
このタスクでは、テンプレートを使って、1 つの仮想ネットワーク、1 つのネットワーク セキュリティ グループ、3 つの仮想マシンをデプロイします。
...
1. **\\Allfiles\\Labs\\06\\az104-06-vms-template.json ** ファイルを見つけて選択し、**[開く]** を選択します。

1. **[保存]** を選択します。

1. **[パラメーターの編集]** を選択して、**\\Allfiles\\Labs\\06\\az104-06-vms-parameters.json** ファイルを読み込みます。
...
1. Azure portal で `Load balancers` を検索して選択し、[+ 作成] をクリックし、ドロップダウン メニューから **[Standard Load Balancer]** を選択します。 **[ロード バランサー]** ブレードで **[+ 作成]** をクリックします。
...
1. **[パブリック IP アドレスの追加]** ポップアップで、次の設定を使用してから **[保存]** を 2 回クリックします。 完了したら、**[次へ: バックエンド プール >]** をクリックします。
...
1. **[バックエンド プール]** タブで、次の設定を使って **[バックエンド プールの追加]** をクリックします (その他は既定値のままにします)。 **[追加]**、**[保存]** の順にクリックします。 **[次へ: インバウンド規則 >]** をクリックします。

    | 設定 | 値 |
    | --- | --- |
    | 名前 | `az104-be` |
    | 仮想ネットワーク | **az104-06-vnet1 (az104-rg6)** |
    | バックエンド プールの構成 | **NIC** |
    | **[追加]** をクリックして仮想マシンを追加します |  |
    | az104-06-vm0 | **チェックボックスをオンにします** |
    | az104-06-vm1 | **チェックボックスをオンにします** |

   > **注:** パブリック IP アドレスを作成するときに、VM がデプロイされているリージョン (北ヨーロッパ) と、リージョンが一致することを確認してください。 ポータルで、米国東部 2 などの別のリージョンが事前に選択されている場合があります。必要に応じて変更してから続行ください。
...
1. **[+ 追加]** を選択します。 次の設定で負荷分散規則を追加します (その他は既定値のままにします)。  規則を構成するときは、情報アイコンを使って各設定について確認します。 終わったら、**[保存]** をクリックします。
    ...
    | セッション永続化 | **なし** |
    | アイドル タイムアウト (分) | `4` |
    | TCP リセットを有効にします | **Disabled** |
    | フローティング IP を有効にする | **Disabled** |
...
1. 次の設定でサブネットを追加します (他の設定は既定値のままにします)。
    ...
    | 名前 | `subnet-appgw` |
    | 開始アドレス| `10.60.3.224` |
    | サイズ | `/27` - **[開始アドレス]** は引き続き **10.60.3.224** です|

1. **[プライベート サブネット]** セクションで、**[プライベート サブネットを有効にする (既定の送信アクセスなし)]** をチェックされたままにします。 

1. **[追加]** をクリックします。

    > **注**:このサブネットは、Azure アプリケーション ゲートウェイによって使われます。 Application Gateway には、/27 以上のサイズの専用サブネットが必要です。

1. Azure portal で「`Application gateways`」を検索して選択し、**[Application Gateway]** ブレードで **[+ 作成]** をクリックします。
...
1. ページの下部の **[次へ: バックエンド >]** をクリックし、**[バックエンド プールの追加]** をクリックします。 次の設定を指定します (他の設定は既定値のままにしておきます)。 完了したら、 **[追加]** をクリックします。
...
1. **[パスベースのルーティング]** セクションで、**[パスベースの規則を作成する複数のターゲットを追加します]** を選択します。 2 つの規則を作ります。 最初のルールの後にある **[追加]** をクリックし、2 番目のルールの後にある **[追加]** をクリックします。 
...
1. 必ず変更を確認してから、**[次へ:タグ >]** を選択します。 変更は不要です。
...
1. **[Application Gateway]** リソースの **[監視]** セクションで、**[バックエンドの正常性]** を選択します。
...
+ Azure portal で、リソース グループを選び、**[リソース グループの削除]**、**[リソース グループ名を入力してください]** の順に選び、**[削除]** をクリックします。 2 回目の確認ダイアログが表示されたら、もう一度 [削除] をクリックします。
