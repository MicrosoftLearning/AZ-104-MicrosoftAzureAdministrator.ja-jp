...
1. 次の設定でピアリングを追加し (他のユーザーには既定値を残します)、**[追加]** をクリックします。
    ...
    | 'az104-06-vnet01' がピアリングされた仮想ネットワークにアクセスできるようにする | **チェック ボックスがオンになっていることを確認します (既定)** |
    | 'az104-06-vnet01' のゲートウェイがピアリングされた仮想ネットワークにトラフィックを転送できるようにする | **チェック ボックスがオンになっていることを確認します** |
    ...
    | az104-06-vnet2 が az104-06-vnet01 にアクセスできるようにする | **チェック ボックスがオンになっていることを確認します (既定)** |
    | az104-06-vnet2 が az104-06-vnet01 から転送されたトラフィックを受信できるようにする | **チェック ボックスがオンになっていることを確認します** |
...
1. 次の設定でピアリングを追加し (他のユーザーには既定値を残します)、**[追加]** をクリックします。
    ...
    | 仮想ネットワークのデプロイ モデル | **リソース マネージャー** |
    | リソース ID を知っている | enabled |
    | Resource ID | このタスクで先ほどメモした **az104-06-vnet3** の resourceID パラメーターの値。 |
    | az104-06-vnet3 が az104-06-vnet01 にアクセスできるようにする | **チェック ボックスがオンになっていることを確認します (既定)** |
    | az104-06-vnet3 が az104-06-vnet01 から転送されたトラフィックを受信できるようにする | **チェック ボックスがオンになっていることを確認します** |
...
1. **[診断テストの実行]** をクリックし、接続チェックの結果が返されるまで待ちます。 状態が **[成功]** になったことを確認します。 ネットワーク パスを確認します。接続が直接であり、仮想マシン間に中間ホップがないことに注意してください。
...
1. **[診断テストの実行]** をクリックし、接続チェックの結果が返されるまで待ちます。 状態が **失敗** であることに注意してください。
...
1. **[Network Watcher - 接続のトラブルシューティング]** ブレードで、次の設定を使用します (その他の設定は既定値のままにします)。
...
1. **[診断テストの実行]** をクリックし、接続チェックの結果が返されるまで待ちます。 状態が **[成功]** になったことを確認します。 ネットワーク パスを確認し、トラフィックが **az104-06-nic0** ネットワーク アダプターに割り当てられた **10.60.0.4** を経由してルーティングされたことを確認します。 状態が **[失敗]** の場合は、az104-06-vm0 を再起動する必要があります。
...
1. 次の設定でロード バランサーを作成し (その他の設定は既定値のままにします)、 **[次へ: フロントエンド IP 構成]** をクリックします。
    ...
    | --- | --- |
    | サブスクリプション | このラボで使用している Azure サブスクリプションの名前 |
    | リソース グループ | **az104-06-rg4** (必要に応じて作成) |
    | 名前 | **az104-06-lb4** |
