...
## タスク 3: テナントを作成する (省略可能 - 上限に関する問題、有料サブスクリプションが必要)

この作業では、新しいテナントを作成します。
    
1. Azure portal で、**Microsoft Entra ID** を検索して選びます。

    >**注**:ラボ環境での Captcha 検証には既知の問題があります。 **[作成に失敗しました。要求が多すぎます。後で試してみてください]** というエラーが表示された場合は、次の操作を行ってください。
    - 作成を数回試してください。<br>
...
1. 前に作成した最初のテナントに戻ります。
2. ナビゲーション ウィンドウで、**概要**を選択します。
3. **[テナントを管理する]** をクリックします。
4. 前に作成した最初のテナントの横にあるチェック ボックスをオンにし、[**切り替え**] を選択します。
