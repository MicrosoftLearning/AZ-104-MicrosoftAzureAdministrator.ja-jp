...
1. リソース グループ ブレードで、左側のメニューの **[タグ]** をクリックし、新しいタグを作成します。
...
1. **[適用]** をクリックし、タグ エディション ウィンドウを閉じて、ストレージ アカウント ブレードに戻ります。 ストレージ アカウント上の省略記号をクリックし、 **[タグの編集]** を選択します。新しいタグがストレージ アカウントに自動的に割り当てられなかったことがわかります。 
...
1. **[ストレージ アカウントの作成]** ブレードの **[基本]** タブで、ポリシーが適用されたリソース グループを使用していることを確認し、次の設定を指定して **[確認]** をクリックして (その他の設定は既定値のままにします)、 **[作成]** をクリックします。

    | 設定 | 値 |
    | --- | --- |
    | ストレージ アカウント名 | 英字で始まる、グローバルに一意な、3 個から 24 個の小文字と数字の任意の組み合わせ |

    >**注**: **[検証に失敗しました。詳細については、ここをクリックしてください]** というエラーが表示されることがあります。その場合は、エラー メッセージをクリックして失敗の理由を特定し、次の手順をスキップします。 
