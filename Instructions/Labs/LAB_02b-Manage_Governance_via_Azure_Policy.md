...
このトピックで役立つ可能性のある対話型ラボ シミュレーションがいくつか存在します。 このシミュレーションを使用すると、同様のシナリオを自分のペースでクリックしながら進めることができます。 対話型シミュレーションとこのラボには違いがありますが、主要な概念の多くが同じです。 Azure サブスクリプションは必要ありません。 
...
1. デプロイが作成されると、ポータルの **[通知]** リストに **"デプロイできませんでした"** というメッセージが表示されるはずです。 **[通知]** リストからデプロイ概要に移動し、"**Deployment failed.Click here for details (デプロイに失敗しました。詳細については、ここをクリックしてください)** " のメッセージをクリックして、失敗の理由を特定します。 
    ...
    >**注**:リソースのデプロイがポリシーによって許可されなかったことをエラー メッセージが示しているのを確認します。 
