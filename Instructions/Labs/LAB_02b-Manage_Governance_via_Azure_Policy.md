...
lab:
  ...
  description: リソース タグを使用するように Azure Policy を構成します。
  duration: 30 minutes
  level: 300
  islab: true
  primarytopics:
...
このタスクでは、Azure portal を介してタグを作成し、Azure リソース グループに割り当てます。 タグは、Microsoft Well-Architected Framework とクラウド導入フレームワークで概説されているように、ガバナンス戦略の重要なコンポーネントです。 タグを使用すると、リソース所有者、期限日、グループ連絡先、さらに組織が重要と見なすその他の名前と値のペアをすばやく識別できます。 このタスクでは、リソース コスト センターを識別するタグを割り当てます。 
...
1. **[次へ]** を選択し、**[タグ]** タブに移動します。新しいタグの情報を入力します。
...
1. `Require a tag and its value on resources` 組み込みポリシーを検索します。 ポリシーを選択し、少し時間を取って定義を確認します。 
...
1. **[次へ]** をクリックし、**[修復]** タブと **[マネージド ID]** タブを確認します。 **[マネージド ID]** タブの **[マネージド ID の作成]** チェックボックスはオフのままにしておきます。 
...
1. ポータルで、`Storage Accounts` を検索して選択し、**[+ 作成]** を選択します。 
...
>**注**: **[未処理エラー]** タブをクリックすると、**"タグとその値がリソースに必要"** というロール定義の名前など、エラーの詳細を確認できます。 デプロイに失敗した原因は、作成しようとしたストレージ アカウントに、値が **Default** に設定された **Cost Center** という名前のタグがなかったためです。
...
1. 割り当てのリストで、**"タグとその値がリソースに必要"** ポリシーの割り当てを表す行の省略記号アイコンをクリックし、**[割り当ての削除]** メニューの項目を使用してこの割り当てを削除します。

1. **[ポリシーの割り当てを削除しますか?]** というメッセージが表示されたら、**[はい]** を選択します。
...
1. 次の警告に注意してください。このリソース グループとその依存リソースの削除は永続的な操作であり、元に戻すことはできません。 **[削除]** を選択します。 2 回目の [削除の確認] ダイアログが表示されたら、もう一度 **[削除]** をクリックして確認します。
...
## 自習トレーニングでさらに学習する
+ [Azure Policy のイニシアティブ](https://learn.microsoft.com/training/modules/sovereignty-policy-initiatives/)。 このモジュールでは、組織の標準を適用するために Azure Policy のイニシアティブを使用し、Azure リソースのコンプライアンスを大規模に評価し、管理する方法について説明します。
