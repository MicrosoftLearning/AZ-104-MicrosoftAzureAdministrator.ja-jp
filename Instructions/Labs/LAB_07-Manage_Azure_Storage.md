...
lab:
  ...
  description: Azure ストレージ アカウント、Blob Storage、ファイル共有を構成します。
  duration: 50 minutes
  level: 400
  islab: true
  primarytopics:
...
1. `Storage accounts` を検索して選択し、結果から **[ストレージ アカウント]** を選択し、**[+ 作成]** をクリックします。
...
1. **[Create a storage account]\(ストレージ アカウントの作成\)** ブレードの **[Basics]** タブで、次の設定を指定します (それ以外は既定値のままにします)。
    ...
    | リージョンを利用できない場合に、データへの読み取りアクセスを行うことができるようにします。 | チェックボックスをオンにします。 |
...
1. **[詳細設定]** と **[セキュリティ]** タブで、情報アイコンを使用して、選択肢の詳細を確認します。 既定値のままにします。
   
1. **[公衆ネットワーク アクセス]** の選択の **[ネットワーク]** タブで、**[無効]** を選択します。 これにより、送信アクセスを許可しながら受信アクセスを制限します。 
...
1. **[確認および作成]** を選択し、検証プロセスが完了するのを待ってから、**[作成]** をクリックします。
...
1. **[セキュリティとネットワーク]** ブレードで、**[ネットワーク]** を選択します。 **公衆ネットワーク アクセス**が無効になっています。

    + [パブリック ネットワーク アクセス] で **[管理]** をクリックして、[パブリック ネットワーク アクセスの構成] ブレードを開きます。
      
    + **[パブリック ネットワーク アクセス]** を **[有効]** に設定します。 **[パブリック ネットワーク アクセス スコープ]** を **[選択したネットワークから有効にする]** に設定します。
      
    + **[リソースの設定: 仮想ネットワーク、IP アドレス、例外]** セクションで、クライアント IPv4 アドレスを追加します。
      
    + 変更を保存。 ネットワーク セキュリティ境界を関連付けることを提案する情報バナーが表示された場合は、無視して続行できます。
  
1. **[データ管理]** ブレードで、**[冗長性]** を選択します。 プライマリ データ センターとセカンダリ データ センターの場所に関する情報があります。

1. **[データ管理]** ブレードで、**[ライフサイクル管理]** を選択してから、**[規則の追加]** を選択します。

    + 規則に `Movetocool` という **名前を付けます**。 規則の範囲を制限するオプションを確認します。 **次へ** をクリックします。 
    
    + **[ルールの追加]** ページで、基本 BLOB が最後に変更されたのが `30` 日より前の "場合は"、**クール アクセス層ストレージに移動します**。**** 他の選択肢に注意してください。 
...
1. **[データ ストレージ]** ブレードで、**[コンテナー]** を選択します。 
...
1. 使用中のコンテナーで、右端にある省略記号 (...) までスクロールし、**[アクセス ポリシー]** を選択します。

1. アカウントに対して共有キーを使用した認可が無効であることを示す警告が表示された場合は、無視して続行できます。 

1. **[不変 BLOB ストレージ]** 領域で、**[ポリシーの追加]** を選択し、種類を **[訴訟ホールド]** から **[時間ベースのリテンション]** に変更します。
...
1. ストレージ アカウントの左側メニューで **[構成]** を選択し、**[ストレージ アカウント キーへのアクセスを許可する]** を **[有効]** に設定して、**[保存]** をクリックします。 

1. 次に、**[アクセス制御 (IAM)]** に移動し、**[ロールの割り当ての追加]** をクリックし、**[ストレージ BLOB データ共同作成者]** ロールと **[ストレージ ファイル データ制限付き共同作成者]** ロールを選択してユーザー アカウントに割り当て、**[レビューと割り当て]** をクリックします。 

1. アクセスが構成されたら、**[データ]** コンテナーを選択し、**[アップロード]** をクリックします。
...
1. アップロード ファイルを選び、**[ダウンロード]**、**[削除]**、**[層の変更]**、**[リースの取得]** などの省略記号 (...) のオプションを確認します。

1. アップロードしたファイルを選択して概要パネルを開き、[プロパティ] テーブルの **[クリップボードにコピー]** ボタンを使用して URL をコピーします。 URL を、新しい **[InPrivate]** ブラウザー ウィンドウに貼り付けます。
...
1. アップロードしたファイルに戻り、右端にある省略記号 (...) を選択してから **[SAS の生成]** を選択します。

1. このアカウントで共有キーを使用した認可が無効になっていることを示す警告バナーに注意してください。この場合、アカウント キーの署名オプションは使用できません。
  
1. 次の設定を指定します (他は既定値のままにします): アップロードしたファイルに戻り、右端にある省略記号 (...) を選択してから **[SAS の生成]** を選択し、次の設定を指定します (他は既定値のままにします)。
...
1. Azure portal でストレージ アカウントに戻り、**[データ ストレージ]** ブレードで **[従来のファイル共有]** をクリックします。

1. **[新しいファイル共有]** をクリックし、**[基本]** タブでファイル共有に `share1` という名前を付けます。 
...
1. ファイル共有が作成されると、バックアップを有効にするように求める情報バナーが表示されます。このメッセージは無視して続行できます。
...
1. **[従来のファイル共有]** を選択し、**share1** ディレクトリが存在することを確認します。

1. **share1** ディレクトリを選択すると、**[+ ディレクトリの追加]** を使用できることがわかります。 これを使用すると、フォルダー構造を作成できます。

1. 認可エラーが表示された場合は、ストレージ ブラウザーのツール バーで **[Azure AD アカウントの切り替え]** を選択します (または、ストレージ ブラウザー ツール バーで認証方法を **[Microsoft Entra ユーザー アカウント]** に変更します)。
...
1. ポータルで、「**ネットワーク基盤**」を検索して選択します。

1. `Virtual networks` で **[作成]** をクリックします。 [基本] タブで **[リソース グループ]** を`az104-rg7` と設定し、仮想ネットワークに `vnet1` という**名前**を付けます。
...
1. **[設定]** セクションで **[サービス エンドポイント]** ブレードを選択します。
    ...
    + **[サービス エンドポイント ポリシー]** ドロップダウンは既定値の **0 が選択された**ままにします。
...
1. **[パブリック ネットワーク アクセス]** で **[管理]** を選びます。 

1. **[仮想ネットワークの追加]**、**[既存のネットワークの追加]** の順に選びます。

1. **vnet1** と **default** サブネットを選んで、**[追加]** を選びます。

1. **[IPv4 アドレス]** セクションで、マシンの IP アドレスを**削除**します。 許可されるトラフィックは、仮想ネットワークからの着信のみになるはずです。 
...
+ Azure portal で、リソース グループを選び、**[リソース グループの削除]**、**[リソース グループ名を入力してください]** の順に選び、**[削除]** をクリックします。 リソース グループとその依存リソースの削除が永続的なアクションであることを示す 2 回目の確認ダイアログが表示されたら、もう一度 **[削除]** をクリックして削除を完了します。
...
+ [ガイド付きプロジェクト - Azure Files と Azure BLOB](https://learn.microsoft.com/training/modules/guided-project-azure-files-azure-blobs/)。 Azure Blob Storage と Azure Files を使用してビジネス データを安全に格納する練習を行います。
