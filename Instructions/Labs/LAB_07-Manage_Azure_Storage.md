...
1. LICENSE.txt ファイルをダウンロードできるはずです。
...
1. **[コマンド スクリプトの実行]** ブレードの **[PowerShell スクリプト]** ペインで以下を実行して、ストレージ アカウントの **az104-07-container** コンテナーから LICENSE BLOB のダウンロードを試みます (`[blob SAS URL]` プレースホルダーを、前のタスクで生成した BLOB SAS URL に置き換えます)。
