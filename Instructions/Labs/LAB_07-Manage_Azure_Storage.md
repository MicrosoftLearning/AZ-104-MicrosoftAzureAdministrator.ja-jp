...
1. Azure portal で、このラボの最初のタスクで作成したストレージ アカウントのブレードに戻り、「**セキュリティ + ネットワーク**」 セクションで、「**ネットワーク**」 をクリックし、「**ファイアウォールと仮想ネットワーク**」 を表示します。

1. 「**選択した仮想ネットワークとIPアドレスから有効**」 オプションをクリックして、このオプションを有効にしたら使用できる構成設定を確認します。
...
1. 「**The MIT License (MIT)**」 ページの内容が表示されるか、ファイルがダウンロードされます。
