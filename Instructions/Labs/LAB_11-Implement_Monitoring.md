...
lab:
  ...
  description: Azure Monitor のアラートとクエリを構成します。
  duration: 40 minutes
  level: 300
  islab: true
  primarytopics:
...
1. **\\Allfiles\\Labs\\11\\az104-11-vm-template.json** ラボ ファイルをコンピューターにダウンロードします。
...
1. 次の情報を使用してカスタム デプロイ フィールドを完了します。他のすべてのフィールドは既定値のままにします。
    ...
    | リージョン        | **米国東部** (デプロイに失敗した場合は、別のリージョンに変更して、もう一度やり直してください。 これは、異なるリージョンのクォータが原因です。) |
...
1. ポータルで、「**仮想マシン**」を検索して選択し、**[az104-vm0]** を選択します。 VM の状態が **[停止済み]** と表示されている場合は、**[開始]** を選択し、状態が **[実行中]** に変わるまで待ちます。
...
1. 仮想マシンの横にある **[有効にする]** を選択します。

1. **[テレメトリ メトリックを開く]** がチェックされて**いない**ことを確認します。 

1. **[確認と有効化]**、**[有効にする]** の順に選択します。

1. 仮想マシン エージェントのインストールとオンボードには数分かかります。

1. 次のタスクに進むことができますが、引き続き通知を監視してください。 オンボードに失敗した場合は、もう一度有効にしてみてください。
...
1. サブスクリプションのボックスをオンにし、**[適用]** を選択します。 このアラートは、サブスクリプション内のすべての仮想マシンに適用されます。 または、特定のマシンを 1 つだけ指定することもできます。 
...
1. アラートに関する作業を続けます。 **[アクション]** タブに移動し、**[アクションの選択]** で **[アクション グループの使用]** を選択してから **[+ アクション グループの作成]** を選択します。

    >**ご存知でしたか?** 警告ルールには、最大 5 つのアクション グループを追加できます。 アクション グループは、特定の順序ではなく、同時に実行されます。 複数の警告ルールで同じアクション グループを使用できます。 
...
1. **[リソースの削除]** ペインの **[削除を確認するために、'delete' を入力してください]** テキスト フィールドに `delete` と入力し、**[削除]** を選びます。 2 回目の **[削除の確認]** ダイアログが表示されたら、もう一度 **[削除]** を選択して確認します。
...
1. 使用している**サブスクリプション**を選択し、**[適用]** を選択します。
...
1. 必要に応じてスプラッシュ スクリーンを閉じます。 

1. スコープのプロンプトが表示されたら、**[サブスクリプション]** を選択し **[適用]** をクリックします。 スコープのセレクターが表示されない場合は、既定のスコープで続行してください。

1. クエリ エディターの右側で、**[簡易モード]** の横にあるドロップダウンを選択し、**[KQL モード]** を選択します。

    ![[クエリ] タブのスクリーンショット。](../media/az104-lab11-queries.png)

1. クエリ エディターで、次のクエリを入力して **[実行]** をクリックします。
...
1. オプションで、上部ツール バーの **[クエリ ハブ]** を選択するとサンプル クエリを閲覧できます。 **[仮想マシン]** カテゴリと **[ハートビートのカウント]** を選択できる場合は、比較のために実行してください。
...
1. クエリを次のように置き換えて、**[実行]** をクリックします。 結果のグラフを確認します。 
   ...
   ```
    ...
    >**注:** クエリが正しく貼り付けられない場合は、メモ帳に貼り付けてから、コピーしクエリ フィールドに貼り付け直してみてください。
...
+ Azure portal で、**[az104-rg11]** リソース グループに移動し、**[リソース グループの削除]** を選択し、リソース グループ名を入力して確認してから、**[削除]** をクリックします。 2 回目の [削除の確認] ダイアログが表示されたら、もう一度 **[削除]** をクリックして削除を完了します。
...
+ [アラートと応答を構成します](https://learn.microsoft.com/training/modules/configure-alerts-responses/)。 潜在的な問題がユーザーの問題になる前にそれに関する通知を事前に管理するために、アラートと応答を構成および管理する方法について理解します。
