...
lab:
  ...
  description: Azure Backup を仮想マシン向けに構成します。
  duration: 50 minutes
  level: 400
  islab: true
  primarytopics:
...
1. **[セキュリティ設定] > [論理的な削除の設定]** ラベルの下にある **[更新]** リンクを選択します。

1. **[論理的な削除の設定]** ブレードで、**[論理的な削除のリテンション期間]** が **14** 日になっていることを確認して、ブレードを閉じます。 
...
1. **[OK]** をクリックしてポリシーを作成してから、**[仮想マシン]** セクションで **[追加]** (下スクロール) を選択します。
...
1. 次の情報を使用してストレージ アカウントを定義してから、**[確認および作成]** を選択します。
...
1. **[監視]** ブレードで **[診断設定]** を選択し、**[診断設定の追加]** を選択します。
...
1. バックアップ ジョブの**詳細を表示します** (リンクのある右にスクロールします)。
...
1. **詳細設定** を選択します。 リソースの選択が行われました。 

1. 下にスクロールし、オートメーション アカウントを**作成**します。 

   >**注:** 事前設定されていることが重要です。さもないと、検証は失敗します。 

1. **[レプリケーションの確認と開始]**、**[レプリケーションを開始する]** の順に選択します。
...
+ Azure portal で、リソース グループを選び、**[リソース グループの削除]**、**[リソース グループ名を入力してください]** の順に選び、**[削除]** をクリックします。 アクションが永続的であり、元に戻すことができないことを示す **[削除の確認]** ダイアログボックスが表示されたら、もう一度 **[削除]** をクリックします。
...
+ 「[Azure Backup の概要](https://learn.microsoft.com/training/modules/intro-to-azure-backup/)」。 Azure Backup の機能によってニーズに合ったバックアップ ソリューションが提供されるしくみについて説明します。
